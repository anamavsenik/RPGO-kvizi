...
% m deCasteljaujevih postopkov za parameteter v
for i=1:m
    b1x(:,i) = bezier(Bx(:,i), v); %navaden deCast. postopek, saj nimamo racionalnih krivulj
    b1y(:,i) = bezier(By(:,i), v);
    b1z(:,i) = bezier(Bz(:,i), v);
end
...
for j=1:N
    bx(j,:) = bezier(b1x(j, :)', u)'; %to pomeni da transponira ne odvaja seveda
    by(j,:) = bezier(b1y(j, :)', u)';
    bz(j,:) = bezier(b1z(j, :)', u)';
end
